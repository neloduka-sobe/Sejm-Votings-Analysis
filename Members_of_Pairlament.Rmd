...
# On List votings
on_list_voting_files <- list.files(votings_path, pattern = "_on_list.csv")

# Yes/No votings
#y_n_voting_files <- list.files(votings_path)
#y_n_voting_files <- y_n_voting_files[!y_n_voting_files %in% on_list_voting_files]
#y_n_voting <- data.frame()

#for (file in y_n_voting_files) {
#  temp_data <- read.csv(paste0(votings_path, file), header=TRUE)
#  y_n_voting <- merge(y_n_voting, temp_data, all=TRUE)
#  rm(temp_data)
# }
```
