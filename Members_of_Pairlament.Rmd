...
```{R}
# https://r-charts.com/evolution/calendar-heatmap/
# Change for the beginning and end of the cadency

votings_per_day <- y_n_votings %>% group_by(date) %>% summarize(number = max(votingNumber))
votings_per_day <- votings_per_day %>% mutate(yday = yday(date))


all_days_df <- data.frame(yday = 1:366) # Change for the beginning and end of the cadency
#merged_df <- merge(all_days_df, birthdays_per_day, by = "yday", all.x = TRUE)
#merged_df$number[is.na(merged_df$number)] <- 0
