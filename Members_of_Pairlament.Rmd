...
```{R}
build_similarity_network <- function(mp_data, voting_data, threshold = 0.5) {
...
visualize_network <- function(graph, leaders_ids) {
  ...
  if (n_clubs > 0) {
    color_palette <- RColorBrewer::brewer.pal(max(3, min(n_clubs, 9)), "Set1")
  ...
  custom_layout <- function(graph, leader_indices) {
    layout <- layout_with_fr(graph, niter = 500)
    
    for (i in 1:50) {
      for (l1 in leader_indices) {
        for (l2 in leader_indices) {
          if (l1 != l2) {
            diff <- layout[l1, ] - layout[l2, ]
            dist <- sqrt(sum(diff^2))
            if (dist < 0.4) {
              layout[l1, ] <- layout[l1, ] + diff / dist * 0.1
              layout[l2, ] <- layout[l2, ] - diff / dist * 0.1
            }
          }
        }
      }
  ...
  layout <- custom_layout(graph, leader_indices)
  ...
  # Add arrows for leader labels
  for (i in leader_indices) {
    if (!is.na(V(graph)$label[i])) {
      coords <- layout[i,]
      arrows(coords[1], coords[2], coords[1]*1.15, coords[2]*1.15, 
             length = 0.1, col = "black", lwd = 1.5)
      text(coords[1]*1.2, coords[2]*1.2, V(graph)$label[i], 
           cex = 0.9, font = 2)
    }
  }

  # Adjust legend to include leaders
  legend_colors <- c(club_colors, "black")
  legend_names <- c(names(club_colors), "Leaders")
  legend_pch <- c(rep(19, length(club_colors)), 15)
...
least_similar <- least_similar %>%
  left_join(mp %>% select(id, firstName, secondName, lastName, club),
            by = c("from" = "id")) %>%
  left_join(mp %>% select(id, firstName, secondName, lastName, club),
            by = c("to" = "id"), suffix = c("_from", "_to"))
