...
```{R}
leaders_ids <- c(148, 400, 241, 122, 174, 58)
```
...
visualize_network <- function(graph, leaders_ids) {
  ...
  V(graph)$size <- 5  # Default size
  V(graph)$size[leader_indices] <- 10  # Larger size for leaders
  ...
  if (length(club_colors) > 0) {
    legend("bottomright", legend = names(club_colors), 
           col = club_colors, pch = 19, pt.cex = 1.5, 
           title = "Clubs", bty = "n")
  }
}
