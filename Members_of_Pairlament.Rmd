...
```{R}
mp <- mp %>% mutate(birthDate = as_date(birthDate))
```
...
```{R, out.width="100%"}
# Remove NA #TODO
ggplot(mp, aes(area=numberOfVotes, fill=club, 
...
# Visualisation using calendar for birthdays # DO IT BETTER
```{R}
birthdays_per_day <- mp %>% group_by(month = month(birthDate), day = (birthDate)) %>% summarise(number=n())

#ggplot(birthdays_per_day, aes(x=wday, y=week)) +
#  geom_tile(aes(fill = number)) +
#  facet_wrap(~month, scales = "free") +
#  scale_y_reverse() +
#  theme_minimal() +
#  scale_x_discrete(position = "top") 
#calendarHeat(as_date(birthdays_per_day$month, birthdays_per_day$day), birthdays_per_day$number)
```
